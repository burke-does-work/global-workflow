...
- One sentence per line unless reduces clarity (for clean diffs)
- Prefer unordered lists to ordered lists
- Use a horizontal rule (---) between sections introduced by level-two headings (##). Example:
> ```markdown
> body text
> 
> ---
...
> ## Next ## section
> ```
- Do not use emphasis for headings. Example: use `## Topic`. Do not use `**topic:`.
- Use "-", not "—" for dash
